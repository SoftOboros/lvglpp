# 00 — LvFontView selection & anti-aliased text
...
on the LPAR-08 `LvFontView` handle ([`../core-draw/00-text-draw-image-mask.md`](../core-draw/00-text-draw-image-mask.md))
...
| --- | --- |
| LvFontView-handle selection model (`WidgetFont`/`set_font` **intent**), AA-coverage contract | rlvgl `v0.2.5` `docs/concepts/FONT-00-CONCEPTS.md` (@ `f999f75`) |
...
| LvFontView as a **style/cascade property** | `lvgl/src/core/lv_obj_style.h` + `lv_obj_style_gen.h` (`lv_obj_set_style_text_font`, `lv_obj_get_style_text_font`), `lv_style_gen.h` (`lv_style_set_text_font`) |
| Existing `LvFontView` handle | `core/include/lvglpp/core/draw_lvgl.hpp` (LPAR-08) — extended here, not replaced |
...
Make any lvglpp `ObjectView` render **selectable, anti-aliased** text. The glyph
...
1. **LvFontView selection** (§5): assign any `lv_font_t` to an object or style via
   the cascade, and resolve the effective font of an object.
2. **AA built-ins** (§6): expose LVGL's built-in anti-aliased
   `lv_font_montserrat_*` fonts and an AA/glyph-format query on the `LvFontView`
...
- The LPAR-08 `LvFontView` handle (`core/include/lvglpp/core/draw_lvgl.hpp:38`) wraps
  `const lv_font_t*` with `default_font()`, `glyph_width()`,
  ...
  advance.
- The LPAR-07 cascade (`set_local_style_prop`, `LvStyle::set_*`) wraps
  `bg`/`border`/`radius`/`text_color` but **not** `text_font`. There is no
  `set_text_font` on `Style` and no `set_local_text_font` /
  `resolved_text_font` helper, so no object can be given a font through the
  wrapper today.
...
- **`LvFontView`** — As defined in `core/include/lvglpp/core/draw_lvgl.hpp:38` (LPAR-08);
...
- **`BuiltinFont`** — Owned by this chapter; an enum naming LVGL's built-in
  `lv_font_montserrat_*` sizes. `LvFontView::builtin(BuiltinFont)` returns the font
  if its `LV_FONT_MONTSERRAT_<n>` build flag is enabled, else an **empty
  `LvFontView`** (so the caller falls back to `default_font()`). **Specification
...
- **`LvStyle::set_text_font` / `set_local_text_font(ObjectView, ...)`** — wrap
  `lv_style_set_text_font` / `lv_obj_set_style_text_font`; the `LvFontView` is
  borrowed (LVGL stores the `lv_font_t*`, so it MUST outlive every object
  using it — the same outlives-objects rule as a shared `LvStyle`).
- **`resolved_text_font(ObjectView, StylePart::Main)`** — wraps `lv_obj_get_style_text_font(obj,
...
1. LvFontView is selected through the **cascade**, never by mutating a widget field:
   `LvStyle::set_text_font(const lv_font_t*)` and
   `set_local_text_font(ObjectView, const lv_font_t*, StyleSelector)` are the only write
   paths; `resolved_text_font(ObjectView, StylePart::Main)` is the resolve/read path.
2. The selected `LvFontView` is **borrowed into LVGL** (the `lv_font_t*` is stored,
   not copied): a `LvFontView` set on a live object MUST outlive that object. This
   is the same load-bearing rule as a shared `LvStyle` (LPAR-07 §5.1);
   built-in fonts (static storage) and a `LvFontView` held by the caller both
   satisfy it.
...
4. lvglpp adds **no** coverage/AA rasterization. AA is a property of the font
   ...
   default font is AA. `LvFontView::is_anti_aliased()` reports whether a
...
5. `BuiltinFont` enumerates the montserrat sizes LVGL can ship
   ...
   `LvFontView` for a size whose `LV_FONT_MONTSERRAT_<n>` flag is off. Which sizes
...
- **CORE-06 `BitmapFont`/`PackedFont`/`FONT_6X10`** — the hand-rolled value
  ...
  When `LVGLPP-WRAP` migrates the widgets onto `ObjectView`, the bring-up
  ...
  value types.
- **LPAR-08 `LvFontView`** — extended in place (new accessors), not replaced; the
  `glyph_width`/`line_height` surface is unchanged.
...
- Per-widget convenience `set_font` on concrete widget wrappers — those land
  with each widget phase; FONT-00 provides the `ObjectView`-level primitive they
  call.
- The `FontId` registry — owned by FONT-05.

## §12 Acceptance checklist

- [x] `LvFontView` gains `builtin(BuiltinFont)`, `glyph_metrics(codepoint)`,
      `is_anti_aliased()`, `base_line()`; empty-safe (return empty/0).
      `core/include/lvglpp/core/draw_lvgl.hpp` + `core/src/draw_lvgl.cpp`.
- [x] `BuiltinFont` enum + `builtin()` returns the compiled-in font or an
      empty `LvFontView` (per-size `#if LV_FONT_MONTSERRAT_<n>`); `default_font()`
      fallback documented in the header + §5.3.
- [x] `LvStyle::set_text_font(const lv_font_t*)`
      (`style_lvgl.hpp`), `set_local_text_font(ObjectView, const lv_font_t*,
      Selector)` + `resolved_text_font(ObjectView, StylePart::Main)` cascade resolution
      (`style_lvgl.{hpp,cpp}`, over `lv_obj_set/get_style_text_font`).
...
- `rlvgl/docs/concepts/FONT-00-CONCEPTS.md` (v0.2.5 @ `f999f75`)
- `lvgl/src/font/lv_font.h`; built-in `lv_font_montserrat_*`
- `lvgl/src/core/lv_obj_style_gen.h`, `lvgl/src/misc/lv_style_gen.h`
- `core/include/lvglpp/core/draw_lvgl.hpp` (LPAR-08 `LvFontView`)
- `core/include/lvglpp/core/style_lvgl.hpp`, `object.hpp` (LPAR-07)

## §14 Unblocks

- FONT-05 (registry feeds these selection setters).
- Every widget phase (per-widget `set_font` calls the `ObjectView` primitive).

## §15 Change log

- **2026-06-15** — FONT-00 drafted: extend the `LvFontView` handle (built-ins,
  glyph metrics, AA query), add cascade font selection
  (`set_text_font`/`set_local_text_font`/`resolved_text_font`). Reframe vs. rlvgl
