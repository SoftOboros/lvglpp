...
wrapper up to the **rlvgl v0.2.5** font-selection surface. It mirrors the
rlvgl `FONT-00` and `FONT-05` concepts families
(`rlvgl/docs/concepts/FONT-*.md` @ `f999f75`) onto upstream LVGL via **RAII /
...
| --- | --- | --- |
| LvFontView selection (FONT-00 §5) | `lv_obj_set_style_text_font` + the style cascade | wraps it: `set_local_text_font(ObjectView, ...)`, `LvStyle::set_text_font`, `resolved_text_font(ObjectView, StylePart::Main)` (cascade resolution) |
| Anti-aliased text (FONT-00 §6) | built-in 4-bit AA `lv_font_montserrat_*`, A1/A2/A4/A8 glyph formats | exposes the AA built-ins + a glyph-format / AA query on the `LvFontView` handle |
...
Gated on **LPAR-08** (the `LvFontView` handle over `lv_font_t`, landed
`docs/core-draw/00-text-draw-image-mask.md`) and **LPAR-07** (the style
cascade `Style`/`set_local_style_prop`, landed
