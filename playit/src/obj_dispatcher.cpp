...
#include "lvglpp/core/input.hpp"  // LPAR-04 event_to_indev (point extraction)
...
Response ObjDispatcher::dispatch(const Command& cmd) noexcept {
    return std::visit([this](const auto& payload) -> Response {
        ...
        if constexpr (std::is_same_v<T, command::Status>) {
            ...
            // matching rlvgl).
            ::lvglpp::core::Event ev = to_event(payload.event);
            lv_indev_data_t data{};
            if (::lvglpp::core::event_to_indev(ev, &data)) {
                lv_obj_t* target = hit_at(root_.borrow_raw(), data.point.x, data.point.y);
                if (target != nullptr) {
